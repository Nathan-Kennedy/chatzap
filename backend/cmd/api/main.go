...
func main() {
	...
	api.Get("/flows/:id", authMW, handler.HandleGetFlow(db))
	api.Patch("/flows/:id", authMW, handler.HandlePatchFlow(log, db))
	api.Delete("/flows/:id", authMW, handler.HandleDeleteFlow(log, db))
