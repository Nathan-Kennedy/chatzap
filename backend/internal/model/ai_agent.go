...
type AIAgent struct {
	...
	TTSProvider       string `gorm:"size:32;default:none;column:tts_provider"` // none | openai_tts | omnivoice | elevenlabs | kokoro
	OpenAITTSVoice    string `gorm:"size:128;column:openai_tts_voice"`         // voz OpenAI / voice_id ElevenLabs / voz Kokoro / clone:… OmniVoice
	OpenAITTSModel    string `gorm:"size:64;column:openai_tts_model"`            // modelo OpenAI TTS (vazio = default servidor)
